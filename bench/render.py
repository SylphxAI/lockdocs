...
def sub(key, line):
    rs = [r for r in rows if r.get("line") == line]
...
table = [
    ...
    "",
    f"| | correct | older majors | newer majors | {', '.join(single) or 'single version'} | median tokens | median latency |",
    "|---|---|---|---|---|---|---|",
]
for key, label in [("fetched", "lockdocs + `lockdocs fetch`"), ("hybrid", "lockdocs, package files only"), ("context7", "Context7 (anonymous)")]:
    ...
    if a:
        table.append(f"| {label} | {a['passed']}/{a['total']} | {sub(key, 'older')} | {sub(key, 'newer')} | {sub(key, 'single')} | {a['median_tokens']:,} | {fmt_ms(a['median_ms'])} |")
