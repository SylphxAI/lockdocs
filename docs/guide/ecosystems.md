...
- **Sources:** `node_modules/<name>` here or in a parent directory, and pnpm's `node_modules/.pnpm/<name>@<version>` store, so the exact version is found even when another is hoisted.
...
- **Sources:** `$CARGO_HOME/registry/src/*/<name>-<version>` (what `cargo fetch` or any build downloads), or `vendor/`.
